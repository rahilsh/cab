...
public class FuberApp {
	...
	public FuberApp(HttpServletRequest request, HttpServletResponse response)
		...
		response.setCharacterEncoding(FuberConstants.CHAR_ENCODING_UTF8);
	...
	private void stop() throws NumberFormatException, JSONException, IOException {
		...
		float distance=Float.parseFloat(DBStore.cabs(Float.parseFloat(lat),Float.parseFloat(lon),false,number).getJSONObject(0).getString("distance"));
	...
	private void book() throws JSONException, IOException {
		...
		String result = null;
		String number = getNearestCab(
				Float.parseFloat(user.get("lat").toString()),
				Float.parseFloat(user.get("lon").toString()), pink);
		if (number != null) {
		...
		response.getWriter().write(
				new JSONObject("{\"result\":\"" + result + "\"}").toString());
	}

	private String getNearestCab(float lat, float lon, boolean pink)
